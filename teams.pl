...
% primer formar els equips i despres anar afegnt restriccions

%makeTeams:- team(T), worker(W), writeClause([ wt(W,T) ]), fail.
%makeTeams.

eachWorkerOneTeam:- worker(W), findall( wt(W,T), team(T), L), exactly(1, L), fail.
eachWorkerOneTeam.

setSize:- minSize(Min), maxSize(Max), team(T), findall( wt(W,T), worker(W), L), atLeast(Min, L), atMost(Max, L), fail.
setSize.
