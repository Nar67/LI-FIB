...
incompatible(S1,S2):- %% returns a pair of students that coincided last year
    ...
    lastYearTeam(L), member(S1, L), member(S2, L), true.
...
writeClauses:- 
	eachMemberOneTeam,
	eachTeamFiveMembers,
	eachMemberCompatible,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachMemberOneTeam:- student(S), findall( st(S, T), team(T), L), exactly(1, L), fail.
eachMemberOneTeam.

eachTeamFiveMembers:- team(T),  teamSize(Size), findall( st(S, T), student(S), L), exactly(Size, L), fail.
eachTeamFiveMembers. 

eachMemberCompatible:- team(T), incompatible(S1, S2), incompatible(S1, S3), S2 \= S3,
