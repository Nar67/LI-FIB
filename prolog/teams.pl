...
incompatibleWorkers(W1,W2):- workerScore(W1,S1), workerScore(W2,S2), W1 < W2,
                            S is S1+S2, maxScore(Max), S > Max.
complexWorkers(W1, W2):- workerScore(W1,S1), workerScore(W2,S2),
                            S is S1+S2, complexThreshold(Max), S >= Max.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 1.- Declare SAT variables to be used
% wt(W,T) means "worker W is in team T"
satVariable( wt(W,T) ):- worker(W), team(T).
satVariable( c(T) ):- team(T). %T is a complex team.
...
writeClauses(MaxComplexTeams):-
    ...
    setIncompatible,
    setComplex,
    atMostMaxComplex(MaxComplexTeams),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.


eachWorkerOneTeam:- worker(W), findall( wt(W,T), team(T), L), exactly(1, L), fail.
eachWorkerOneTeam.

setSize:- minSize(Min), maxSize(Max), team(T), findall( wt(W,T), worker(W), L), atLeast(Min, L), 
            atMost(Max, L), fail.
setSize.

setIncompatible:- team(T), incompatibleWorkers(W1, W2),
                  writeClause([ -wt(W1, T), -wt(W2, T)]), fail.
setIncompatible.

setComplex:- worker(W1), worker(W2), W1 < W2,
             team(T), complexWorkers(W1,W2), writeClause([ -wt(W1,T), -wt(W2,T), c(T)]), fail.
setComplex.

atMostMaxComplex(MaxComplexTeams):- findall( c(T), team(T), L), atMost(MaxComplexTeams, L), fail.
atMostMaxComplex(_).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
