...
satVariable( modd(M, D) ):- moderator(M), day(D).
...
eachEventOneDay:-	event(E), findall(ed(E,D),( eventDays(E,L), member(D,L) ), Lits),  
					exactly(1, Lits), fail.
eachEventOneDay.

eachEventOneMod:-   event(E), findall(em(E,M),(eventModerators(E,Mods), member(M,Mods)), L), 
					exactly(1, L), fail.
eachEventOneMod.

eachDayMaxEvents:-	day(D), findall(ed(E,D), event(E), L), maxEventsPerDay(Max), atMost(Max, L), fail.
eachDayMaxEvents.

eachModMaxDays:- moderator(M), findall(modd(M,D), day(D), L), maxDaysPerModerator(Max), atMost(Max, L), fail.
eachModMaxDays.

relacionaVars:- event(E), moderator(M), day(D), 
				writeClause([ -ed(E,D), -em(E,M), modd(M,D)]), fail.
relacionaVars.
