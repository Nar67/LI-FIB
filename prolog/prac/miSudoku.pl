...
filledInputValues:- entrada(Sud), %llegeix entrada
					nth1(I,Sud,Row), %posa a row el valor del Iessim de Sud
					nth1(J,Row,K), %el mateix pero a k
					integer(K), % si K es un enter
					writeClause([ x(I,J,K) ]), % escrivim la clausula a la fila I columna J hi ha el valor K 
					fail. % es per mirar tota l'entrada
filledInputValues. % 

eachIJexactlyOneK:- row(I), col(J), % donada una I i J
					findall( x(I,J,K), val(K), Lits ), % Posa a Lits totes les expressions del tipus x(I,J,K) on K varia
					exactly(1,Lits), fail. % de Lits, nomes agafa 1
eachIJexactlyOneK.
