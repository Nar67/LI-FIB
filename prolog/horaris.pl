...
eachCourseOneDay:- course(C), findall( cd(C,D), day(D), L ), exactly(1,L), fail.
eachCourseOneDay.

eachCourseNHours:-  course(C), day(D), courseHours(C, N), 
                    findall( cdh(C,D,H), hour(H), L), exactly(N, L).
eachCourseNHours.

eachCourseOneRoom:- course(C), findall( cr(C,R), room(R), L), exactly(1,L), fail.
eachCourseOneRoom.

eachProfessorOneCourse:- professor(P), findall( cp(C,P), course(C), L), exactly(1,L), fail.
eachProfessorOneCourse.

eachCourseOneRoom:-
eachCourseOneRoom.

eachCourseAvaliableProfessor:-
eachCourseAvaliableProfessor.

eachCourseAvaliableRooms:-
eachCourseAvaliableRooms.

avoidOverlappingCourses:-   course(C1), course(C2), day(D), hour(H) C1 < C2, 
                            courseYear(C1, Y1), courseYear(C2, Y2), Y1 == Y2, 
                            writeClause([cdh(C1, D, H), cdh(C2, D, H)]), fail.
avoidOverlappingCourses.
